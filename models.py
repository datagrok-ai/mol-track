...
class Batch(Base):
    ...
    updated_by = Column(UUID(as_uuid=True), nullable=False)
    compound_id = Column(
        Integer, ForeignKey(f"{DB_SCHEMA}.compounds.id"), nullable=False
    )
...
class BatchDetail(Base):
    ...
    property_id = Column(
        Integer, ForeignKey(f"{DB_SCHEMA}.properties.id"), nullable=False
    )

    value_qualifier = Column(
...
class AssayTypeDetail(Base):
    ...
    assay_type_id = Column(
        Integer, ForeignKey(f"{DB_SCHEMA}.assay_types.id"), primary_key=True
    )
    property_id = Column(
        Integer, ForeignKey(f"{DB_SCHEMA}.properties.id"), primary_key=True
    )
...
class AssayType(Base):
    ...
    property_requirements = relationship(
        "AssayTypeProperty", back_populates="assay_type"
    )
...
class Assay(Base):
    ...
    id = Column(Integer, primary_key=True, index=True)
    assay_type_id = Column(
        Integer, ForeignKey(f"{DB_SCHEMA}.assay_types.id"), nullable=False
    )
    name = Column(Text, nullable=False)
    description = Column(Text)
    created_at = Column(DateTime(timezone=True), server_default=func.now())
