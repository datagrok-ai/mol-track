...
def test_update_institution_id_pattern_valid(client, api_headers):
    # Test with valid pattern
    response = client.patch(
        "/v1/admin/settings",
        data={"name": enums.SettingName.CORPORATE_COMPOUND_ID_PATTERN.value, "value": "DG-{:09d}"},
        headers=api_headers,
    )
...
def test_update_institution_id_pattern_invalid_format(client, api_headers):
...
def test_update_compound_matching_rule_already_set(client, api_headers):
...
def test_update_compound_matching_rule_success(client, api_headers):
