...
def test_update_compound_matching_rule_already_set(client):
    ...
    assert response.status_code == 200
    assert (
        f"Compound matching rule is already set to {CompoundMatchingRule.ALL_LAYERS.value}"
...
def test_update_compound_matching_rule_success(client):
    ...
    assert response.status_code == 200
    assert (
        f"Compound matching rule updated from {CompoundMatchingRule.ALL_LAYERS.value} to {CompoundMatchingRule.STEREO_INSENSITIVE_LAYERS.value}"
        in response.json()["message"]
    )
