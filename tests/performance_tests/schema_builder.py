...
from app.utils.enums import PropertyClass, EntityType, ValueType


class SchemaBuilder:
    def __init__(self, output_dir: Path, config: Dict[str, Any], value_types: Dict[EntityType, list[ValueType]]):
    ...
    def generate_schema_and_mapping(self) -> Dict[str, Dict[str, Any]]:
        """Generate schema and mapping files for every entity_type."""

        writer = FileWriter(self.output_dir)
        for entity_type, rule in self.schema_generating_rules.items():
            schema = {"properties": [], "identity": [], "identity_cols": []}
            mapping = {}

            self._generate_identity_properties(entity_type, rule, schema, mapping)
            self._generate_random_properties(entity_type, rule, schema, mapping)

            writer.write_json(f"schemas/{entity_type.value}_schema.json", {"properties": schema["properties"]})
            writer.write_json(f"{entity_type.value}_mapping.json", mapping)
    ...
    def _generate_identity_properties(
        self, entity_type: EntityType, rule: Dict[str, Any], schema: Dict[str, Any], mapping: Dict[str, str]
    ) -> None:
        ...
        for i, prop_name in enumerate(property_names):
            ...
            prop = self._generate_property(entity_type, prop_name, prop_type)
    ...
    def _generate_random_properties(
        ...
        num_properties = self.details_columns.get(entity_type.value, 1)
        alias = rule["alias"]
        for i in range(num_properties):
            prop_name = f"{entity_type.value}_detail_name_{i + 1}"
            prop = self._generate_property(entity_type, prop_name)
            schema["properties"].append(prop)
            mapping[prop_name] = f"{alias}.{prop_name}"

    def _generate_property(self, entity_type: EntityType, name: str, value_type: ValueType = None) -> Dict[str, Any]:
        ...
        return {
            ...
            "value_type": value_type or random.choice(self.value_types[entity_type]),
            "unit": "",
        }
