...
class SmilesGenerator:
    ...
    def __init__(self, size: int = 1000):
        ...
        with open("./tests/performance_tests/rgroups_1000.smi", "r") as f:
        ...
        self._smiles_generator = self.generate_combinatorial_library(
            core_smiles_list=self.core_smiles_list,
    ...
    def generate_combinatorial_library(
        self, core_smiles_list, rgroups1, rgroups2, rgroups3, limit=None
    ) -> Generator[str, None, None]:
        """
        Generate a combinatorial library of SMILES strings by attaching R-groups to cores.
        """
        core_list = [Chem.MolFromSmiles(core_smiles) for core_smiles in core_smiles_list]
        for core in core_list:
            AllChem.EmbedMolecule(core)
        choices = [i for i in range(len(core_list))]
        ...
        for r1, r2, r3 in product(r1_mols, r2_mols, r3_mols):
            mol = Chem.Mol(core_list[random.choices(population=choices)[0]])

            # note that you need to add in reverse order of the rgroups to preserve the order of the attachment points
            mol = self.attach_rgroup(mol, r3, 2)
            mol = self.attach_rgroup(mol, r2, 1)
            mol = self.attach_rgroup(mol, r1, 0)
            ...
            if smiles in self.unique:
                continue
            else:
                self.unique.add(smiles)
                yield smiles

    def attach_rgroup(self, core_mol, rgroup, attach_idx) -> Chem.Mol:
        """
        ...
        # Create bond
        combo_edit.AddBond(core_neighbor, rgroup_attach + rgroup_offset - 1, order=Chem.rdchem.BondType.SINGLE)

        return combo_edit.GetMol()

    core_smiles_list = [
        # Aromatic cores
        "[*:1]c1ccc(c([*:2])c1)[*:3]",  # Biphenyl-based core
        "[*:1]c1cc([*:2])ccc1[*:3]",  # Phenylene-based core
        "[*:1]c1cc([*:2])cc([*:3])c1",  # Benzene-meta-substituted core
        "[*:1]Oc1ccc([*:2])cc1[*:3]",  # Phenoxy-based core
        # Heterocyclic cores
        "[*:1]c1cncc([*:2])c1[*:3]",  # Pyrimidine-based core
        "[*:1]c1cc([*:2])c(F)nc1[*:3]",  # Fluorinated pyridine core
        # Fused ring systems
        "[*:1]c1cc([*:2])c2cccnc2c1[*:3]",  # Quinoline-based core
        "[*:1]c1cc2c(cc1[*:2])CC([*:3])CC2",  # Tetralin-like core
        # More complex aromatic cores
        "[*:1]c1cc(OC)c([*:2])cc1[*:3]",  # Methoxy-substituted benzene
        "[*:1]c1cc(F)c([*:2])cc1[*:3]",  # Fluorinated benzene
        "[*:1]c1cc(Cl)c([*:2])cc1[*:3]",  # Chlorinated benzene
        # Aromatic-aliphatic mixed cores
        "[*:1]CC(c1ccc([*:2])cc1)[*:3]",  # Benzyl-like core
    ]
