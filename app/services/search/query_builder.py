...
class QueryBuilder:
    ...
    def build_query(self, request: models.SearchRequest) -> Dict[str, Any]:
        ...
        if request.filter:
            ...
            if(sql_components['where'] or sql_components['having']):
                ...
                filter_sql = (
                    ...
                    f"FROM {from_clause} "
                    f"{filter_joins} "
                    f"{where_clause} " "{connect} "
                    f"{group_by_clause} "
                    f"{having_clause}) "
                )
                # The inner query needs to have a different alias compared to the outter query
                filter_sql = filter_sql.replace(f"{table_config['alias']}.",f"{table_config['alias']}{table_config['alias']}.")
        ...
        return {
            'sql': complete_sql.strip(),
            'params': query_params
        }
    
    def build_base_sql_parts(self, output_list: List[str], search_level: str, all_joins: JoinOrderingTool) -> Dict[str, Any]:
        """
    ...
    def build_filter_sql_parts(self, filter_obj: models.Filter, level: str, all_joins: JoinOrderingTool) -> Dict[str, Any]:
        """
    ...
    def build_condition(self, condition: models.AtomicCondition, level: str, all_joins: JoinOrderingTool) -> Dict[str, Any]:
        """
