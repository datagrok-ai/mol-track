...
class SearchOperators:
    ...
    def get_sql_expression(cls, operator: str, field: str, value: Any, threshold: float = None) -> Tuple[str, Dict[str, Any]]:
        ...
        if operator == "ON":
            ...
            param_names = [f"param{i}" for i in range(len(value))]
    ...
    @classmethod
    def get_operators_for_type(cls, data_type: str) -> List[str]:
        """Get list of valid operators for a given data type"""
        type_mapping = {
            "string": [op for op, def_ in cls.OPERATORS.items() if def_["type"] == OperatorType.STRING],
            "int": [op for op, def_ in cls.OPERATORS.items() if def_["type"] in [OperatorType.NUMERIC, OperatorType.STRING]],
            "double": [op for op, def_ in cls.OPERATORS.items() if def_["type"] in [OperatorType.NUMERIC, OperatorType.STRING]],
            "datetime": [op for op, def_ in cls.OPERATORS.items() if def_["type"] in [OperatorType.DATETIME, OperatorType.STRING]],
            "bool": ["=", "!="],
            "molecular": [op for op, def_ in cls.OPERATORS.items() if def_["type"] == OperatorType.MOLECULAR]
        }
